...
namespace GenericTableAPI.Services
{
    public abstract partial class SyntaxService
    {
            ...
            string? orderBy = null, int? limit = null, int? offset = null, string? connectionString = null)
        {
            ...
            switch (dbType)
            {
                case DatabaseType.SqlServer:
                    ...
                    {
                        query += $" OFFSET @offsetParam ROWS";
                        if (limit.HasValue)
                        {
                            query += $" FETCH NEXT @limitParam ROWS ONLY";
                        }
                    }
                    break;
        ...
        {
            ValidateTableName(tableName);
            ValidateColumnName(primaryKeyColumn);
            
            tableName = GetTableName(tableName, schemaName);
            DatabaseType dbType = GetDatabaseType(null);
            
            string paramPrefix = dbType == DatabaseType.Oracle ? ":" : "@";
            return $"SELECT * FROM {tableName} WHERE {primaryKeyColumn} = {paramPrefix}idParam";
        }

        /// <summary>
        /// Validates column names to ensure they don't contain SQL injection attempts
        /// </summary>
        ...
        {
            if (!string.IsNullOrEmpty(columnName) && !TableNameRegex.IsMatch(columnName))
            {
        ...
        public static string AddQuery(string tableName, string? schemaName, IDictionary<string, object?> values, string? columns = null, string? strValues = null, string? primaryKeyColumn = null, string? connectionString = null)
        {
            ...
            // Validate all column names in the values dictionary
            ...
            string paramPrefix = databaseType == DatabaseType.Oracle ? ":" : "@";
            var parameterNames = string.Join(", ", values.Keys.Select(k => $"{paramPrefix}{k}"));

            string sql;
            switch (databaseType)
            {
                case DatabaseType.SqlServer:
                    sql = $"INSERT INTO {tableName} ({columnNames}) OUTPUT Inserted.{primaryKeyColumn} VALUES ({parameterNames})";
                    break;

                case DatabaseType.Oracle:
                    sql = $"INSERT INTO {tableName} ({columnNames}) VALUES ({parameterNames}) RETURNING {primaryKeyColumn} INTO :retVal";
                    break;
        ...
        public static string UpdateQuery(string tableName, string? schemaName, string id, IDictionary<string, object> values, string? primaryKeyColumn = null, string? setClauses = null)
        {
            ...
            var setClause = string.Join(", ", values.Keys.Select(k => $"{k} = {paramPrefix}{k}"));

            string sql = $"UPDATE {tableName} SET {setClause} WHERE {primaryKeyColumn} = {paramPrefix}idParam";
        ...
        public static string DeleteQuery(string tableName, string? schemaName, string id, string? primaryKeyColumn = null)
        {
            ...
            string paramPrefix = dbType == DatabaseType.Oracle ? ":" : "@";
            string sql = $"DELETE FROM {tableName} WHERE {primaryKeyColumn} = {paramPrefix}idParam";
        ...
        /// <summary>
        /// Constructs a parameterized SQL SELECT query to retrieve column names from a table.
        /// </summary>
        /// <param name="tableName">The name of the table.</param>
        /// <param name="schemaName">The name of the schema (optional).</param>
        /// <param name="connectionString">The connection string.</param>
        ...
        public static string GetColumnsQuery(string tableName, string? schemaName, string? connectionString = null)
        {
            ...
            {
                case DatabaseType.SqlServer:
                    sql = $"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = {paramPrefix}tableNameParam";
                    break;
                case DatabaseType.Oracle:
                    sql = $"SELECT COLUMN_NAME FROM ALL_TAB_COLS WHERE TABLE_NAME = :tableNameParam";
                    break;
                case DatabaseType.Unknown:
                default:
                    throw new NotSupportedException();
